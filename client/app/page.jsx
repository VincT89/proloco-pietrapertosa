...
export default async function Home({ searchParams }) {
  ...
  const eventiVerificati = data.eventi.items.filter(ev => ev.status === 'verified');
  const comunitaVerificata = data.comunita.items.filter(com => com.status === 'verified');
  const notizieVerificate = data.notizie.items.filter(not => not.status === 'verified');
          ...
          <div className="ed-wrap">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '60px' }}>
              <div>
                ...
                <h2 className="ed-title" style={{ marginBottom: 0 }}>{data.eventi.titolo}</h2>
              </div>
          ...
          <div className="ed-wrap">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '60px' }}>
              <div>
                ...
                <h2 className="ed-title" style={{ marginBottom: 0 }}>{data.notizie.titolo}</h2>
              </div>
            </div>
                ...
                <Link key={notizia.id} href={`/notizie#avviso-${notizia.id}`} style={{ textDecoration: 'none' }}>
                  <div className="ed-list-item" style={{ padding: '20px 0', borderBottom: '1px solid var(--hair)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'background 0.3s', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '30px', flexWrap: 'wrap' }}>
                      <div style={{ color: 'var(--gold)', fontFamily: '"Cormorant Garamond", serif', fontSize: '1.1rem', minWidth: '100px' }}>{notizia.data}</div>
                      <div>
                        <h4 style={{ fontSize: '1.3rem', fontFamily: '"Cormorant Garamond", serif', color: 'var(--paper)', marginBottom: '4px', fontWeight: 400 }}>{notizia.titolo}</h4>
          ...
          <div className="ed-wrap">
            <div className="ed-split" style={{ alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                ...
                <h2 className="ed-title" style={{ marginBottom: '30px', fontSize: 'clamp(2.5rem, 5vw, 4.5rem)' }}>{data.scopri.titolo}</h2>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 40px 0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {scopriVerificati.map(scopri => (
                    <li key={scopri.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ color: 'var(--paper)', fontSize: '1.3rem', fontFamily: '"Cormorant Garamond", serif' }}>{scopri.nome}</span>
                    </li>
                  ))}
                </ul>
